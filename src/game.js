...
const elements = {
  ...
  tapRipple: document.querySelector("#tapRipple"),
...
function playTone({ frequency = 220, duration = 0.12, type = "sine", volume = 0.16, slide = 1 }) {
...
function playSound(type) {
  const soundMap = {
    mine: () => playTone({ frequency: 118 + state.drillLevel * 8, duration: 0.09, type: "triangle", volume: 0.12, slide: 0.62 }),
    sell: () => playTone({ frequency: 420, duration: 0.18, type: "sine", volume: 0.13, slide: 1.55 }),
    cool: () => playTone({ frequency: 330, duration: 0.2, type: "sine", volume: 0.1, slide: 0.72 }),
    upgrade: () => {
      playTone({ frequency: 360, duration: 0.12, type: "sine", volume: 0.12, slide: 1.22 });
      window.setTimeout(() => playTone({ frequency: 540, duration: 0.16, type: "sine", volume: 0.1, slide: 1.12 }), 70);
    },
    reward: () => {
      playTone({ frequency: 520, duration: 0.12, type: "sine", volume: 0.12, slide: 1.25 });
      window.setTimeout(() => playTone({ frequency: 780, duration: 0.18, type: "sine", volume: 0.1, slide: 1.05 }), 95);
    },
    error: () => playTone({ frequency: 140, duration: 0.16, type: "triangle", volume: 0.09, slide: 0.84 }),
  };
  soundMap[type]?.();
}
...
function showTapRipple() {
  elements.tapRipple.classList.remove("active");
  void elements.tapRipple.offsetWidth;
  elements.tapRipple.classList.add("active");
}

function spawnFloater(text, color = "#ffc857") {
  state.floaters.push({
    text,
    x: 320 + Math.random() * 130,
    y: 245 + Math.random() * 60,
    vy: -1.15 - Math.random() * 0.5,
    life: 70,
    color,
  });
}
...
function spawnParticles(amount, intense = false) {
  ...
  if (intense) {
    state.beams.push({ x: 380, y: 310, radius: 12, life: 36, color: layer.color });
  }
}
...
function mine(multiplier = 1) {
  ...
  state.combo = now < state.comboUntil ? Math.min(5, state.combo + 0.12) : 1;
  state.comboUntil = now + 1600;
  const comboBonus = 1 + (state.combo - 1) * 0.08;
  const mined = Math.ceil((state.drillLevel * 2 + state.scannerLevel + 2) * multiplier * comboBonus);
  ...
  spawnParticles(12 + Math.floor(state.combo * 2), multiplier > 1 || state.combo > 2.5);
  spawnFloater(`+${format(mined)} Erz`, layer.color);
  showTapRipple();
  bumpElement(elements.ore);
  playSound("mine");
  ...
  if (nextLayer.name !== layer.name) {
    ...
    spawnFloater(nextLayer.name, nextLayer.color);
    playSound("reward");
  }
...
function sellOre() {
  ...
  spawnFloater(`+${format(revenue)} Credits`, "#5cf2a9");
  bumpElement(elements.credits);
  playSound("sell");
  state.ore = 0;
  updateUI();
}
...
function buyUpgrade(type) {
  ...
  spawnParticles(26, true);
  spawnFloater("Upgrade!", "#ffc857");
  bumpElement(elements.credits);
  playSound("upgrade");
  updateUI();
}
...
function completeContracts() {
  ...
  spawnFloater("Auftrag fertig!", "#5cf2a9");
  playSound("reward");
...
function updateUI() {
  ...
  elements.comboText.textContent = `Combo x${state.combo.toFixed(1)}`;
  elements.missionHint.textContent = `Sammle ${format(Math.max(0, contract.ore - state.minedForContract))} Erz für: ${contract.title.replace("Auftrag: ", "")}.`;
...
function drawBackground(width, height) {
  ...
  for (let star = 0; star < 52; star += 1) {
    const x = (star * 97 + animationFrame * 0.15) % width;
    const y = 24 + ((star * 53) % 145);
    const pulse = 0.35 + Math.sin(animationFrame / 25 + star) * 0.25;
    ctx.fillStyle = `rgba(223, 252, 255, ${pulse})`;
    ctx.beginPath();
    ctx.arc(x, y, star % 5 === 0 ? 1.8 : 1.05, 0, Math.PI * 2);
    ctx.fill();
  }
...
function drawBeams() {
  ...
  for (const beam of state.beams) {
    beam.radius += 8;
    beam.life -= 1;
    ctx.globalAlpha = Math.max(0, beam.life / 36) * 0.45;
...
function drawFloaters() {
  ...
  for (const floater of state.floaters) {
    ...
    floater.life -= 1;
    ctx.globalAlpha = Math.max(0, floater.life / 70);
...
function tick(time) {
  ...
  if (performance.now() > state.comboUntil) {
...
elements.mineButton.addEventListener("click", () => {
  pressButton(elements.mineButton);
  mine(1);
});
...
elements.sellButton.addEventListener("click", () => {
  pressButton(elements.sellButton);
  sellOre();
});
elements.coolButton.addEventListener("click", () => {
  pressButton(elements.coolButton);
  coolDrill();
});
elements.drillButton.addEventListener("click", () => {
  pressButton(elements.drillButton);
  buyUpgrade("drill");
});
elements.crewButton.addEventListener("click", () => {
  pressButton(elements.crewButton);
  buyUpgrade("crew");
});
elements.scannerButton.addEventListener("click", () => {
  pressButton(elements.scannerButton);
  buyUpgrade("scanner");
});
elements.soundButton.addEventListener("click", () => {
  state.soundEnabled = !state.soundEnabled;
  elements.soundButton.textContent = state.soundEnabled ? "🔊 Sound sanft" : "🔇 Sound aus";
